...
public final class GeneralCommands {
    ...
    public static void list(final CommandContext args, final CommandSender sender) throws CommandException {
        ...
        new SimplePaginatedResult<ServerStatus>(Settings.get().getNetwork(), MAXPERPAGE) {
            ...
            @Override
            public void display(WrappedCommandSender sender, List<? extends ServerStatus> results, int page) throws CommandException {
            ...
            StatusCollection.get().getServers().values().stream()
                .filter(s -> !s.getName().startsWith(".") || (args.hasFlag('h')  && ServerLinker.hasPerms(sender)))
