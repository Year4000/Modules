...
public final class BungeeConnector implements PluginMessageListener {
    ...
    public void register() {
        Bukkit.getMessenger().registerOutgoingPluginChannel(DuckTape.get(), CHANNEL);
        Bukkit.getMessenger().registerIncomingPluginChannel(DuckTape.get(), CHANNEL, this);
    }

    /** Send data and return a callback */
    public void send(String[] data, Callback<ByteArrayDataInput> back) {
        if (Bukkit.getOnlinePlayers().size() == 0) {
            DuckTape.debug("No players online can not send data");
            return;
        }
        ...
        player.sendPluginMessage(DuckTape.get(), CHANNEL, out.toByteArray());
